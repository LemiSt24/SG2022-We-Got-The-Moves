...
public class FullBodyPoseEmbedder {
  private static final List<String> landmark_names =
      Arrays.asList(
          "nose",
          "left_eye_inner",
          "left_eye",
  ...
  private static int embedding_size =
  ...
  public static double[] generate_embedding(
      LandmarkProto.NormalizedLandmarkList landmarks, Boolean flipped) {
    assert landmarks.getLandmarkCount() == landmark_names.size();
    MyNormalizedLandmark[] normalized_landmarks =
        normalize_landmarks(convert_from_mediapipe(landmarks));
    if (flipped) {
  ...
  private static MyNormalizedLandmark[] convert_from_mediapipe(
    ...
    for (LandmarkProto.NormalizedLandmark landmark : landmarks.getLandmarkList()) {
      MyNormalizedLandmark normalized =
          new MyNormalizedLandmark(landmark.getX(), landmark.getY(), landmark.getZ());
  ...
  private static double get_size(MyNormalizedLandmark[] landmarks) {
    ...
    MyNormalizedLandmark shoulder_center =
        left_shoulder
            .add(right_shoulder)
            .divide(
                2.); // neue Variable eigentlich überflüssig, da left_shoulder das nun auch enthält
  ...
  private static MyNormalizedLandmark get_average_by_names(
      MyNormalizedLandmark[] landmarks, String name_from, String name_to) {
  ...
  private static double get_distance_by_names(
      MyNormalizedLandmark[] landmarks, String name_from, String name_to) {
    MyNormalizedLandmark from = landmarks[landmark_names.indexOf(name_from)];
    MyNormalizedLandmark to = landmarks[landmark_names.indexOf(name_to)];
  ...
  // maybe todo das hier in die MyNormalizedLandmark-Klasse auslagern (ist ja eigentlich nur
  // Subtraktion)
