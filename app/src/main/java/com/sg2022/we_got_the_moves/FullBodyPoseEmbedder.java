...
public class FullBodyPoseEmbedder {
    ...
    private static final int embedding_size = 20; // Diese Zahl muss, wenn man das Embedding verändert, natürlich angepasst werden
    ...
    private static double[] _generate_embedding(NormalizedLandmark[] landmarks)
    {
        ...
        ret[1] = NormalizedLandmark.getAverageByNames(landmarks, "left_shoulder", "left_wrist").getDistance(landmarks[NormalizedLandmark.landmark_names.indexOf("left_elbow")]);

        ret[2] = NormalizedLandmark.getDistanceByNames(landmarks, "right_shoulder", "right_wrist");
        ret[3] = NormalizedLandmark.getAverageByNames(landmarks, "right_shoulder", "right_wrist").getDistance(landmarks[NormalizedLandmark.landmark_names.indexOf("right_elbow")]);

        ret[4] = NormalizedLandmark.getDistanceByNames(landmarks, "left_hip", "left_ankle");
        ret[5] = NormalizedLandmark.getAverageByNames(landmarks, "left_hip", "left_ankle").getDistance(landmarks[NormalizedLandmark.landmark_names.indexOf("left_knee")]);
                
        

        ret[6] = NormalizedLandmark.getDistanceByNames(landmarks, "right_hip", "right_ankle");
        ret[7] = NormalizedLandmark.getAverageByNames(landmarks, "right_hip", "right_ankle").getDistance(landmarks[NormalizedLandmark.landmark_names.indexOf("right_knee")]);
        ...
        // Body bent direction
        ret[18] = NormalizedLandmark.getAverageByNames(landmarks, "left_wrist", "left_ankle").getDistance(landmarks[NormalizedLandmark.landmark_names.indexOf("left_hip")]);
        ret[19] = NormalizedLandmark.getAverageByNames(landmarks, "right_wrist", "right_ankle").getDistance(landmarks[NormalizedLandmark.landmark_names.indexOf("right_hip")]);
