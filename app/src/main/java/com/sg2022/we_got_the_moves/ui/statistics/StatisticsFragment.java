...
public class StatisticsFragment extends Fragment {
  ...
  public void buildBarChart(BarChart barChart) {
    ...
    xAxis.setAxisMinimum(
        0 + 0.5f); // to center the bars inside the vertical grid lines we need + 0.5 step
    xAxis.setAxisMaximum(
        entries.size()
            + 0.5f); // to center the bars inside the vertical grid lines we need + 0.5 step
    ...
    // rightAxis.setLabelCount(4, true); //draw y labels (Y-Values) for 4 horizontal grid lines
    // starting from 0 to 6000f (step=2000)
