...
public class WorkoutListAdapter extends RecyclerView.Adapter<WorkoutListAdapter.WorkoutItemViewHolder> {
    ...
    public WorkoutListAdapter(@NonNull Fragment owner, @NonNull WorkoutsViewModel model) {
        ...
        this.model.data.observe(owner, list -> {
            if (list == null)
                list = new ArrayList<>();
    ...
    public WorkoutItemViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ItemWorkoutBinding binding = DataBindingUtil.inflate(LayoutInflater.from(parent.getContext()), R.layout.item_workout, parent, false);
    ...
    private void showDeleteDialog(Workout w) {
        ...
        builder.setTitle(String.format(this.owner.getString(R.string.delete_workout_titel), w.name))
    ...
    private void showCopyDialog(Workout w) {
        AlertDialog.Builder builder = new AlertDialog.Builder(this.owner.getContext());
        builder.setTitle(String.format(this.owner.getString(R.string.copy_workout_titel), w.name))
                .setMessage(R.string.copy_workout_message)
                .setPositiveButton(R.string.yes, (dialog, id) -> {
                    Workout copy = new Workout(0, w.name);
                    this.model.repository.insertWorkout(copy, new SingleObserver<Long>() {
                        ...
                        public void onSuccess(@NonNull Long aLong) {
                            model.repository.getAllWorkoutExercise(w.id, new MaybeObserver<List<WorkoutExercise>>() {
    ...
    private void showEditDialog(Workout w) {
        AlertDialog.Builder builder = new AlertDialog.Builder(this.owner.getContext());
        TextInputDialogBinding binding = DataBindingUtil.inflate(LayoutInflater.from(this.owner.getContext()), R.layout.text_input_dialog, null, false);
        binding.setWorkout(w);
        builder.setView(binding.getRoot());
        builder.setTitle(String.format(this.owner.getString(R.string.set_workout_title), w.name))
                .setPositiveButton(R.string.yes, (dialog, id) -> {
    ...
    private void showAddDialog(Workout w) {
        AlertDialog.Builder builder = new AlertDialog.Builder(this.owner.getContext());
        this.model.repository.getAllNotContainedExercise(w.id, new MaybeObserver<List<Exercise>>() {
            @Override
            public void onSubscribe(@NonNull Disposable d) {
            ...
            public void onSuccess(@NonNull List<Exercise> list) {
                if (list.isEmpty()) {
                    ...
                    builder.setMultiChoiceItems(items, checkedList, (dialog, which, isChecked) -> checkedList[which] = isChecked);
                    builder.setTitle(String.format(owner.getString(R.string.select_exercises), w.name));
